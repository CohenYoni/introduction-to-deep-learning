...
TOP_SIMILARITIES = 1
CNN_GRAPH_PATH = 'training_cnn_history_graph.png'
...
def get_train_and_test_data_for_cnn():
    """
...
def cnn_pre_processing(dataset, word_vectors):
    """
...
def cnn_training(train_data, train_labels, test_data, test_labels, model_file_path):
    """
    Training cnn model
...
def train_task_handler(user_args):
    ...
    # train, test = get_train_and_test_data_for_cnn()
    # logging.info('loading vectors...')
    # word_vectors = load_fast_text_vectors(user_args.wordvec)
    # logging.info('performing cnn train pre processing...')
    # train_data, train_labels = cnn_pre_processing(train, word_vectors)
    # logging.info('performing cnn test pre processing...')
    # test_data, test_labels = cnn_pre_processing(test, word_vectors)
    # logging.info('start training cnn..')
    # cnn_training(train_data, train_labels, test_data, test_labels, user_args.model)
    # logging.info('train cnn task done!')
